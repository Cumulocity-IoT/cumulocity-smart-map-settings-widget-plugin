...
export class ImageRotateService {
    ...
    /**
     * This service is used to load extended plugin for image rotaion using 3 smart point rotation
     */
    initialize(L: any) {
        ...
        // SVG Overaly Rotated
        L.svgOverlay.Rotated = L.SVGOverlay.extend({
 ...
 */
            ...
            _reset: function () {
                const div = this._image;
                const divG = this._image.querySelector('g');
                ...
                div.style.width = size.x + 'px';
                div.style.height = size.y + 'px';
               // L.DomUtil.setPosition(divG, pxBounds.min);

              /*   divG.style.width = size.x + 'px';
                divG.style.height = size.y + 'px'; */

                const imgW = this._rawImage.width;
                const imgH = this._rawImage.height;
               /*  const svgArea = this._rawImage.querySelector('g');
                const imgW = svgArea.width;
                const imgH = svgArea.height;
 */
                if (!imgW || !imgH) {
                    return;	// Probably because the image hasn't loaded yet.
                }
                ...
                // svgArea.style.transformOrigin = '0 0';

                // The transformation is an affine matrix that switches
                // coordinates around in just the right way. This is the result
                // of calculating the skew/rotation/scale matrices and simplyfing
                // everything.
/*                 this._rawImage.style.transform = 'translate(' +
                    (vectorX.x / imgW) + ', ' + (vectorX.y / imgW)  + ')';
 */               /*  this._rawImage.style.transform = 'matrix(' +
                    (vectorX.x / imgW) + ', ' + (vectorX.y / imgW) + ', ' +
                    (vectorY.x / imgH) + ', ' + (vectorY.y / imgH) + ', ' +
        ...
        L.svgOverlay.rotated = function (imgSrc, topleft, topright, bottomleft, options) {
            return new L.svgOverlay.Rotated(imgSrc, topleft, topright, bottomleft, options);
        };
